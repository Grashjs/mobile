...
export default function CustomersScreen({ navigation }: RootStackScreenProps<'VendorsCustomers'>) {
  ...
  useEffect(() => {
    if (hasViewPermission(PermissionEntity.WORK_ORDERS)) {
      dispatch(getCustomers({ ...criteria, pageSize: 10, pageNum: 0, direction: 'DESC' }));
    }
  }, [criteria]);
  ...
  const onQueryChange = (query) => {
    onSearchQueryChange<Customer>(query, criteria, setCriteria, setSearchQuery, [
      'name',
      'address',
      'phone',
      'email',
      ...
      <ScrollView style={styles.scrollView}
        ...
        {!!customers.content.length ? customers.content.map(customer => (
          <Card style={{ padding: 5, marginVertical: 5, backgroundColor: 'white' }} key={customer.id}
                onPress={() => navigation.navigate('WODetails', { id: customer.id })}>
            <Card.Content>
              <View style={{ ...styles.row, justifyContent: 'space-between' }}>
                <View style={{ ...styles.row, justifyContent: 'space-between' }}>
                  <View style={{ marginRight: 10 }}>
                    <Tag text={`#${customer.id}`} color='white' backgroundColor='#545454' />
                  </View>
                </View>
              </View>
